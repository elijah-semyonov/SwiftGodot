...
final class VariantCollectionTests: GodotTestCase {
    func testAppendingElementStoresInArray() {
        ...
        XCTAssertEqual(Int(variant), 111, "After appending an Int with the value of 111, the first variant in the \(GArray.self) should hold an Int with the value 111")
    }
    ...
    func testInitWithElementsStoresInArray() {
        ...
        XCTAssertEqual(Int(variant), 333, "After initializing with an Int with the value of 333, the first variant in the \(GArray.self) should hold an Int with the value 333")
    }
    ...
    func testArrayCanBeReassigned() {
        ...
        XCTAssertEqual(Int(variant), 999, "After being replaced with a \(GArray.self) whose first variant wraps 999, the first element should hold an Int with a value of 999")
        
        guard let variant1 = sut.array[1] else {
            XCTFail()
            return
        }
        XCTAssertEqual(Int(variant1), 1111, "After being replaced with a \(GArray.self) whose second variant wraps an Int of 1111, the second element should hold an Int with a value of 1111")
    }
